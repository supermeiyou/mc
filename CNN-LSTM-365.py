...
if __name__ == "__main__":
    ...
    for exp in range(N_EXPERIMENTS):
        ...
        pred_365, truth_365, mse_365, mae_365 = forecast(model, test_scaled)
        ...
        plt.title(f"CNN+LSTM 预测未来365天 Global Active Power - 实验{exp+1}")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"image/CNN-LSTM-365-{exp+1}.png")
        plt.close()
    ...
    # plt.legend()
    # plt.tight_layout()
    # plt.savefig('image/CNN-LSTM-365-bar.png')
    # plt.close()

    # print("Finished! -> image/CNN-LSTM-365-bar.png") 